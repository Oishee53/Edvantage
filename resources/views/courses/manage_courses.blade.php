...
<body>
  ...
  <aside class="w-64 bg-white min-h-screen shadow-md fixed left-0 top-0">
    ...
    <nav class="mt-10 flex flex-col">
      <a href="/admin_panel" class="py-3 px-6 text-primary hover:bg-primaryLightHover hover:text-white font-medium">Dashboard</a>
      <a href="/admin_panel/manage_courses" class="py-3 px-6 text-primary bg-primaryLightHover text-white font-medium">Manage Course</a>
      <a href="/admin_panel/manage_user" class="py-3 px-6 text-primary hover:bg-primaryLightHover hover:text-white font-medium">Manage User</a>
      <a href="/admin_panel/manage_resources" class="py-3 px-6 text-primary hover:bg-primaryLightHover hover:text-white font-medium">Manage Resources</a>
    </nav>
  </aside>
  ...
  @auth
    <div class="flex items-center gap-4 mb-4">
      <form action="{{ route('admin.courses.index') }}" method="GET" class="flex-1">
        <input
          type="text"
          name="q"
    ...
    @if(isset($courses))
      @if(request('q') && $courses->isEmpty())
        <p class="text-center font-bold text-gray-600 py-8">
          No courses found for “{{ request('q') }}”.
        </p>
      @else
  ...
  @else
    <p class="text-center text-gray-700">You are not logged in. <a href="/login" class="text-primary hover:underline">Go to Login</a></p>
  @endauth
</section>
